...
function renderPickerRow({
  ...
  iconHtml = null,
  iconClass = 'avatar',
  infoClass = 'person-row-info',
  title = '',
  titleHtml = null,
  titleClass = 'modal-row-title',
  subtitle = '',
  subtitleHtml = '',
  subtitleClass = 'modal-row-sub',
  plusHtml = '＋',
  plusClass = 'person-row-plus',
  actionsHtml = '',
  rowClass = 'person-row',
} = {}) {
  const iconContent = iconHtml != null ? iconHtml : escapeHtml(String(icon || ''));
  const titleContent = titleHtml != null ? titleHtml : escapeHtml(String(title || ''));
  const subtitleContent = subtitleHtml || (subtitle ? escapeHtml(String(subtitle)) : '');
  const actionNode = actionsHtml || (plusHtml ? `<div class="${escapeAttr(plusClass)}">${plusHtml}</div>` : '');
  return `<div class="${escapeAttr(rowClass)}"${id ? ` id="${escapeAttr(id)}"` : ''}${onclick ? ` onclick="${escapeAttr(onclick)}"` : ''}>
    ...
    <div class="${escapeAttr(infoClass)}">
      <div class="${escapeAttr(titleClass)}">${titleContent}</div>
      ${subtitleContent ? `<div class="${escapeAttr(subtitleClass)}">${subtitleContent}</div>` : ''}
    </div>
    ${actionNode}
  </div>`;
}
